...
SELECT department, 
       ...
       SUM(CASE WHEN termdate <= CURDATE() AND termdate >= '1000-01-01' THEN 1 ELSE 0 END) AS terminated_count, 
       SUM(CASE WHEN termdate IS NULL OR termdate <= '1000-01-01' THEN 1 ELSE 0 END) AS active_count,
       (SUM(CASE WHEN termdate <= CURDATE() AND termdate >= '1000-01-01' THEN 1 ELSE 0 END) / COUNT(*)) AS termination_rate
FROM hr
WHERE age >= 18
GROUP BY department
